...
| 作用 | 解析当前打开的 **Modelica `.mo` 源码** 中的图形相关 `annotation`，在 **WebView** 中渲染示意图（非 OpenModelica 级仿真） |
...
宿主进程（Node，扩展）读取文本 → 自定义解析器产出结构化 `DiagramModel` → `postMessage` 发给 WebView → 浏览器端用 SVG 绘制。
...
- `updatePreview`：`fs.readFileSync` 或传入的内存文本 → `parseModelicaFile` → `postMessage`。
...
主要字段：`DiagramModel.className`、`diagram` / `icon`（`LayerAnnotation`：`coordinateSystem` + `graphics[]`）、`components[]`（`Placement`）、`connections[]`（`connect` + `Line`）。
