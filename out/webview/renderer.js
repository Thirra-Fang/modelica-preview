...
(() => {
  ...
  function absoluteToLocal(p, origin, rotDeg) {
    if (rotDeg === 0)
      return [p[0] - origin[0], p[1] - origin[1]];
  ...
  function pointNearBounds(p, b, tol = 1.5) {
    return p[0] >= b.x1 - tol && p[0] <= b.x2 + tol && p[1] >= b.y1 - tol && p[1] <= b.y2 + tol;
  }
  function nearestEdgeMidpoint(b, toward) {
    const mids = [
      [b.x1, (b.y1 + b.y2) / 2],
      [b.x2, (b.y1 + b.y2) / 2],
      [(b.x1 + b.x2) / 2, b.y1],
  ...
  function orthogonalStub(from, to) {
    const mid1 = [to[0], from[1]];
    const mid2 = [from[0], to[1]];
  ...
  function adjustConnectionForComponents(conn, components) {
    ...
    if (snappedStart) {
      ...
      const targetStart = fromComp ? nearestEdgeMidpoint(componentBounds(fromComp), absPoints[1] ?? oldStart) : oldStart;
      const stub = orthogonalStub(targetStart, oldStart);
      absPoints.splice(0, 1, ...stub);
    }
    if (snappedEnd) {
      const oldEnd = absPoints[absPoints.length - 1];
      const targetEnd = toComp ? nearestEdgeMidpoint(componentBounds(toComp), absPoints[absPoints.length - 2] ?? oldEnd) : oldEnd;
